...
class DeepDreamLLMTrainer:
    def __init__(
        ...
        self.model, self.optimizer, self.autoencoder = accelerator.prepare(model, optimizer, self.autoencoder)
        ...
        if randomize_sentences:
            self.sentences = gen_sentences(self.model, self.tokenizer, device=self.device)
    ...
    def calc_loss(self, sentence1, sentence2):
        ...
        input_ids_1 = self.tokenizer.encode(sentence1, return_tensors="pt").to(
            self.device
        )
        embeddings_1 = self.model.transformer.wte(input_ids_1)
        input_ids_2 = self.tokenizer.encode(sentence2, return_tensors="pt").to(
            self.device
        )
    ...
    def train_autoencoder(
        ...
        for epoch in pbar:
            ...
            loss = torch.mean(
                torch.norm(original_embeddings - reconstructed_embeddings, dim=2)
            )
            ...
            if epoch % print_every == 0:
                if use_openai:
                    similarity = get_sentence_similarity(
    ...
    def optimize_for_neuron_whole_input(
        ...
        sentence = generate_sentence(
            self.model, self.tokenizer, max_length=50, seed=seed
        )
        ...
        for i in tqdm(range(num_iterations), position=0, leave=True):
            ...
            if i % (num_iterations // 30) == 0:
                ...
                reconstructed_sentence = unembed_and_decode(self.model, self.tokenizer, embeddings)[0]
